...
import { getVouchers } from "@/actions/admin";
...
export default function VouchersPage() {
  ...
  const handleSuccess = () => { setModalOpen(false); refresh(); };
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs min-w-[700px]">
            <thead className="bg-slate-50 border-b border-black/5 font-bold text-slate-500">
              <tr><th className="p-4">Kode</th><th className="p-4">Nama</th><th className="p-4">Nilai</th><th className="p-4 text-right">Terpakai / Kuota</th><th className="p-4">Segmen</th><th className="p-4">Berlaku s.d.</th></tr>
            </thead>
            <tbody className="divide-y divide-black/5">
              ...
              items.length===0 ? <tr><td colSpan={6} className="p-6 text-center text-slate-400">Tidak ada voucher yang cocok.</td></tr> :
              items.map((v:any)=><tr key={v.id} className="hover:bg-slate-50/50 transition-colors">
                ...
                <td className="p-4 text-slate-500">{v.target_segment}</td>
      ...
      <VoucherModal open={modalOpen} onClose={closeModal} onSuccess={handleSuccess} />
    </div>
  );
}
